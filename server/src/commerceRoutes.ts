...
const RESERVED_SERVICE_PATHS = new Set([
  ...
  "/v1/agents/info",
  "/v1/opsec/simulate",
  "/v1/opsec/analyze-contract",
  "/v1/opsec/spending-report",
...
function openapiSpec(): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/v1/escrow": { get: { summary: "List escrows with optional filters (buyer, merchant, status)" } },
      "/v1/escrow/info": { get: { summary: "Escrow contract address and ABI" } },
      "/v1/escrow/record": { post: { summary: "Record an on-chain escrow creation" } },
      "/v1/escrow/{id}": { get: { summary: "Get escrow status (on-chain + local)" } },
      "/v1/escrow/{id}/dispute": { post: { summary: "Initiate a dispute on an escrow" } },
      "/v1/escrow/{id}/resolve": { post: { summary: "Arbitrator resolves a disputed escrow" } },
      "/v1/escrow/{id}/claim": { post: { summary: "Merchant claims released escrow" } },
      "/v1/agents": { get: { summary: "List all registered agents with reputation" } },
      "/v1/agents/info": { get: { summary: "Agent registry contract address and ABI" } },
      "/v1/agents/register": { post: { summary: "Register an agent identity" } },
      "/v1/agents/{address}": { get: { summary: "Get agent profile and reputation" } },
      "/v1/agents/{address}/reputation": { post: { summary: "Update agent reputation (gateway admin)" } },
      "/v1/opsec/simulate": { post: { summary: "Dry-run transaction simulation for safety checks" } },
      "/v1/opsec/analyze-contract": { post: { summary: "Analyze a contract address for risk" } },
      "/v1/opsec/spending-report": { get: { summary: "Agent spending summary with budget tracking" } },
    },
  };
}
